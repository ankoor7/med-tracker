...
--
-- Mirrors the client change in src/core/cloudRecord.ts (validateEventInstance +
-- validateEventType). Keep validate_record in lock-step with the TS rules; the
-- pgTAP suite (events_test.sql) covers parity so drift is caught.
--
-- Three additive, optional fields — nothing existing changes shape; no new
-- record type (the fields ride the existing `eventInstance`/`eventType`
-- payloads):
--
--  1. `eventInstance.medId` — the medication the user attributes this event
--     to. Optional free-text id, validated generically as a string when
--     present, same as `doseLog.skipReason` / `regimenChange.note`.
--
--  2. `eventInstance.doseLogEntryId` — the specific logged dose occurrence,
--     when the event was logged from a dose row. Same optional-string rule.
--
--  3. `eventType.category` — a UI hint ('flare' | 'side-effect') for which
--     logging affordance to offer. Optional; when present it must be one of
--     the known values, validated strictly against the vocabulary (mirroring
--     how `regimenChange.kind` is checked against its fixed set), not
--     permissively like `RegimenFieldChange.key` — see cloudRecord.ts.
--
-- Settled (spec §7 Q1): this validator does NOT check that `medId` /
-- `doseLogEntryId` resolve to an existing record, and never will in the
-- current design. `validate_record` is declared `immutable` and evaluates one
-- record at a time with no cross-record lookup, so hard-rejecting a dangling
-- reference is not implementable here. The client resolver
-- (core/sideEffects.ts:validateEventAttribution) is authoritative for
-- referential integrity; the server accepts the optional strings and an
-- unresolved reference is simply treated as unattributed. No new record type
-- is introduced, so the `record_type` enum needs no change.

-- Re-define validate_record. The body is the Stage 18 (0010) function with the
-- eventInstance and eventType branches extended; every other branch is
-- unchanged.
...
begin
  ...
  if t = 'medication' then
    ...
    if p ? 'category' and
       (jsonb_typeof(p->'category') is distinct from 'string'
        or (p->>'category') not in ('flare', 'side-effect')) then
      return 'eventType.category invalid';
    end if;
    ...
    if p ? 'medId' and jsonb_typeof(p->'medId') is distinct from 'string' then
      return 'eventInstance.medId must be a string';
    end if;
    if p ? 'doseLogEntryId' and jsonb_typeof(p->'doseLogEntryId') is distinct from 'string' then
      return 'eventInstance.doseLogEntryId must be a string';
    end if;
