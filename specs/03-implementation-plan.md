...
| 7 | History & Visualisation | 2 | Charts, adherence, export; blood-level chart via extension ~~(superseded by Stage 18 FR-18.11 — never surfaced in the UI)~~ |
...
7. **History & Visualisation.** Detailed history, adherence charts, and a blood-level chart that renders the extension's output; JSON/CSV export and import. **Superseded by Stage 18 FR-18.11:** the blood-level chart is removed as a user-facing concept — `levelSeries` remains a developer-facing extension seam in `core/pharmacology.ts` only, never rendered or advertised in the app.
