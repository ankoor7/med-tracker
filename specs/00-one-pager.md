...
**Working name:** SteadyDose · **Type:** Local-first, encrypted PWA · **Status:** Spec / pre-build
...
## Data rights & privacy
End-to-end encrypted: the cloud stores only ciphertext. Data lives in the user's own AWS account, deployed via infrastructure-as-code. No shared backend, no third-party data custody.

## Tech at a glance
React + TypeScript PWA · IndexedDB (offline source of truth) · Web Crypto E2E encryption · AWS CDK stack (Cognito + API Gateway + Lambda + DynamoDB + S3/CloudFront) in the user's own account.
