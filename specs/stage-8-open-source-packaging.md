...
## 2. Scope
**In:** one-command deploy + generated config; setup/teardown docs; LICENSE + contributing/security policy; secrets guidance; documented pharmacology extension guide + example; first-run onboarding (create account + sign in; optional on-device lock); minimal release hardening.
**Out:** hosted multi-tenant offering; app-store distribution (PWA install only).

## 3. Prerequisites
Deployable backend (Stage 3), data model + security hardening (Stage 4), sync (Stage 5).
...
- FR-8.3. Docs let a new user deploy, create their account, sign in (optionally enabling MFA and the on-device lock), configure meds/schedule, and start logging.
...
- **Docs set:** `README` (what/why/quickstart), `DEPLOY.md` (prereqs: AWS account, SSO/profile, Node; step-by-step; costs; teardown), `SECURITY.md` (credential handling, the **non-zero-knowledge** security model — TLS, KMS at rest, per-user authorization, optional MFA + on-device lock — and threat notes), `EXTENSION.md` (the `DoseAdjustmentStrategy`/`levelSeries` contract + example implementation), `LICENSE` (e.g. MIT).
- **Onboarding:** first-run wizard — deploy-config check → create user / sign in → optionally enable MFA and the on-device lock → optional sample data.
...
3. Implement the first-run onboarding wizard (create account / sign in; optional MFA + on-device lock).
...
- AC5. Given onboarding, when completed, the user has a working account and is signed in (and, if chosen, MFA and the on-device lock are enabled).
...
- Reminder limits and the **non-zero-knowledge** trade-off (the backend can read your data; secured by auth, TLS, and KMS) are stated plainly in docs (no over-promising).
