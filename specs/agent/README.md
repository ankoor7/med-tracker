...
| [`agent-stage-5-self-tuning-doctrine.md`](agent-stage-5-self-tuning-doctrine.md) | Doctrine ledger, per-run audit, pruning, empirical model-role fit | Draft — runs last |
...
Their tooling is testable without spending tokens and already is: `pnpm agent:test` runs 89
Tier-0 tests over the ratchet, bootstrap, preflight, outer loop, and run report — every loop
mechanic driven by a stub agent (`scripts/agent/__tests__/stub-agent.sh`) so a trial run can
only fail on defect parity and directive quality, never on a loop bug.
