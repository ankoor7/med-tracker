...
**Model selection is the easiest control to lose.** The baseline ran an `opus` orchestrator over
`sonnet-5` roles. A trial that silently promotes a role to opus will show a cost change that has
...
| Run | Config | Purpose |
| --- | --- | --- |
| 0 | baseline (already measured, 2026-07-28) | reference |
